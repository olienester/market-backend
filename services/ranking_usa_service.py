...
def get_all_usa_tickers():
    ...
    try:
        ...
        # URL oficial da NASDAQ com todos os símbolos (CSV)
        url = "http://www.nasdaqtrader.com/dynamic/SymDir/nasdaqtraded.txt"
        s = requests.get(url).content
        df = pd.read_csv(io.BytesIO(s), sep='|')
        
        # Limpeza: Remove ETFs e Warrants (focando em empresas reais)
        # ETF = 'E' na coluna 'ETF'. Test Issue = 'Y' na coluna 'Test Issue'.
        df = df[df['ETF'] == 'N'] 
        df = df[df['Test Issue'] == 'N']
        
        # Pega apenas os Símbolos
...
def fetch_bulk_fundamentals(tickers_list):
    ...
    for i in range(0, total, batch_size):
        ...
        try:
            ...
            if tickers_data.empty: continue
            ...
            for ticker in valid_tickers:
                try:
                    ...
                    if mkt_cap < 500_000_000: continue 
...
def get_relatorio_geral_usa():
    ...
    # Verifica Cache (Importante pois esse processamento leva minutos)
    ...
    def ajuste_setor(row):
        ...
        if any(s in setor for s in SETORES_BEST_EN): return row['score_barsi'] * 0.8
